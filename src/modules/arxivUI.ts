...
function getHjfyActivatorFromEvent(event: Event): Element | null {
  const path =
  ...
  for (const node of path) {
    ...
    const activator = (node as Element).closest(`[${HJFY_URL_ATTR}]`);
    if (activator) {
      return activator;
    }
  }

  return getEventElement(event.target)?.closest(`[${HJFY_URL_ATTR}]`) || null;
}

function handleHjfyActivation(event: Event): void {
  const activator = getHjfyActivatorFromEvent(event);
  const url = activator?.getAttribute(HJFY_URL_ATTR)?.trim() || "";
  if (!url) {
    return;
  }
...
function handleHjfyMouseUp(event: MouseEvent): void {
  if (event.button !== 0) {
    return;
  }
  handleHjfyActivation(event);
}

function handleHjfyKeyDown(event: KeyboardEvent): void {
...
export class ArxivUIFactory {
  ...
  static registerMainWindow(win: _ZoteroTypes.MainWindow) {
    ...
    win.document.addEventListener("mouseup", handleHjfyMouseUp, true);
    win.document.addEventListener("keydown", handleHjfyKeyDown, true);
  ...
  static unregisterMainWindow(win: Window) {
    win.document.removeEventListener("mouseup", handleHjfyMouseUp, true);
    win.document.removeEventListener("keydown", handleHjfyKeyDown, true);
  }
